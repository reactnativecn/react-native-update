...
## 2026-08-12 救砖实测:能救什么、救不了什么(Android 真机 + 生产服务端)

两轮实测,同一套 forceBoot 通道,结论截然相反,差别只在崩溃时刻。

**能救(已验证)**:markSuccess 之后 10s 才崩的砖。
时间线:proc_start → 9.4s 原生检测 commit `set for next launch` → 10.0s JS 崩溃 →
下次启动进入修复版并 markSuccess。救援仅领先崩溃点 567ms。

**救不了(已验证)**:markSuccess 后写 storage 标记、下次启动读到标记即崩的砖
(用户提出的理想场景,崩溃点约 proc_start+230ms)。连续 4 次冷启动,原生检测
一次都没能走到 commit,设备停留在 "keeps stopping"。

**根因(不是延迟参数问题)**:5s 固定延迟只是其一,检查+下载本身实测还需 ~4.4s。
即使把延迟归零也远超 230ms。更本质的是原生检测线程与 JS 同进程,JS 崩溃时
进程整体被杀,该轮下载进度全部丢弃,下次启动从零开始——无论重启多少次都无法收敛。

**可行方向(未实施,需三端评估)**:
1. 下载断点续传 + 跨启动累积:每次启动补几百毫秒,多次启动后凑齐并激活(治本,改动最大)
2. 原生检测移入独立进程:JS 崩溃不再连坐(Android 可行;iOS 无此模型)
3. 上次启动异常检测 → 本次启动同步阻塞式检查(牺牲启动速度换救援,需谨慎)
